...
import { useWaterBillUpload } from '../../Context/Energy Profile/SubStep2/Water Bill Upload Context';
...
const SubStep2: React.FC = () => {
  const { addFiles, removeFile } = useWaterBillUpload();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (newFiles) {
        Array.from(newFiles).forEach(file => {
          addBill({ name: file.name, size: formatFileSize(file.size), type: 'water' });
        });
    ...
    // Filter addresses for water bill type
    const waterAddresses = addresses.filter(addr => {
      const facilityAddr = getAddressById(addr.id);
      return facilityAddr?.billType?.includes('water');
    });
