...
import { updateAnnualEnergySpend } from "../../../../store/slices/annualEnergySpendSlice";
...
const SubStep2: React.FC = () => {
  ...
  const annualEnergySpend = useSelector(
    (state: any) => state.annualEnergySpend,
  );

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    if (name === "otherLabel") {
      dispatch(updateAnnualEnergySpend({ [name]: value }));
  ...
  const renderCurrencyField = (
    ...
    >
      ...
      <Tooltip
        title={`Enter the total annual spend on ${label.toLowerCase()}`}
        placement="top-end"
        arrow
      >
        <TextField
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", gap: 0 }}>
        ...
        >
          {renderCurrencyField(
            "Electricity",
            "electricity",
            "Used for verifying unit prices and presenting pre/post design costs",
          )}
          {renderCurrencyField(
            "Natural Gas",
            "naturalGas",
          ...
          >
            <Tooltip
              title="Used for verifying unit prices and presenting pre/post design costs"
              placement="left-end"
              arrow
            >
              <Typography
